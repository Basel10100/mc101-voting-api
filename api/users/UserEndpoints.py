...
from db.DBModels import (CandidateDBModel, NoteDBModel, TokenBlacklist,
                         UserDBModel, VoteDBModel)
from utils.constants import Endpoints, ResponseMessages
from utils.crypto_notes import decrypt_note, encrypt_note
from utils.security import (create_access_token, decode_access_token,
                            hash_password, oauth2_scheme, verify_password)

from .NoteSchemas import NoteCreateSchema, NoteResponseSchema, NoteUpdateSchema
from .UserSchemas import (CandidateSchema, PasswordChangeSchema,
                          UserLoginResponseSchema, UserLoginSchema,
                          UserRegisterResponseSchema, UserSchema,
                          UserUpdateSchema, VoteCountResponseSchema,
                          VoteResponseSchema, VotingSchema)
...
def create_user(user: UserSchema, db=Depends(get_db)):
    ...
    existing_user = (
        db.query(UserDBModel).filter(UserDBModel.email == user.email).first()
    )
    if existing_user:
        raise HTTPException(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            detail="The candidate already exists!",
...
def change_password(
    body: PasswordChangeSchema, payload=Depends(decode_access_token), db=Depends(get_db)
):
...
def vote_candidate(
    candidate: VotingSchema, user=Depends(decode_access_token), db=Depends(get_db)
):
    # Check there is a candidate
    existing_candidate = (
        db.query(CandidateDBModel)
        .filter(CandidateDBModel.id == candidate.candidate_id)
        .first()
    )
    if not existing_candidate:
        raise HTTPException(
            detail="Candidate not found", status_code=status.HTTP_404_NOT_FOUND
        )
    # Check the user has not already voted
    existing_vote = (
        db.query(VoteDBModel)
        .filter(
            VoteDBModel.user_id == user["user_id"],
            VoteDBModel.candidate_id == candidate.candidate_id,
        )
        .first()
    )
    if existing_vote:
        raise HTTPException(
            detail="User has already voted", status_code=status.HTTP_400_BAD_REQUEST
        )
...
# Notes endpoints
@UserRouter.post(
    Endpoints.NOTES_ROOT,
    status_code=status.HTTP_201_CREATED,
    response_model=NoteResponseSchema,
)
def create_note(
    note: NoteCreateSchema, payload=Depends(decode_access_token), db=Depends(get_db)
):
    # Validate personal encryption requirements
    if note.personal_encryption and not note.password:
        raise HTTPException(
            status_code=400, detail="Password required for personal encryption"
        )

    ciphertext, nonce, salt, personal = encrypt_note(
        note.content, note.password if note.personal_encryption else None
    )
    db_note = NoteDBModel(
    ...
    try:
        content_plain = decrypt_note(
            db_note.content_encrypted,
            db_note.nonce,
            db_note.kdf_salt,
            db_note.personal_encryption,
            note.password,
        )
    except Exception:
...
def get_note(
    ...
    db_note = (
        db.query(NoteDBModel)
        .filter(NoteDBModel.id == note_id, NoteDBModel.user_id == payload["user_id"])
        .first()
    )
    if not db_note:
    ...
    try:
        ...
        if db_note.personal_encryption:
            raise HTTPException(
                status_code=401, detail="Invalid or missing password for personal note"
            )
        else:
...
def update_note(
    ...
    if updated.content is not None:
        personal_flag = (
            db_note.personal_encryption
            if updated.personal_encryption is None
            else updated.personal_encryption
        )
        ...
        ciphertext, nonce, salt, personal = encrypt_note(
            updated.content, updated.password if personal_flag else None
        )
...
def get_vote_counts(db=Depends(get_db)):
    results = (
        db.query(
            VoteDBModel.candidate_id, CandidateDBModel.name, func.count(VoteDBModel.id)
        )
        .join(CandidateDBModel, VoteDBModel.candidate_id == CandidateDBModel.id)
        .group_by(VoteDBModel.candidate_id, CandidateDBModel.name)
        .order_by(VoteDBModel.candidate_id)
        .all()
    )
    ################ IMPORTANT ################
