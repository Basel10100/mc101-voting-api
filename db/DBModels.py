...
class UserDBModel(Base):
    ...
    id = Column(
        ColumnNames.ID, Integer, primary_key=True, index=True, autoincrement=True
    )
    ...
    is_active = Column(
        ColumnNames.IS_ACTIVE, Boolean, default=True
    )  # 1 for active, 0 for inactive
    created_at = Column(
        ColumnNames.CREATED_AT, TIMESTAMP(True), default=func.now(), nullable=False
    )
...
class NoteDBModel(Base):
    ...
    user_id = Column(
        ColumnNames.USER_ID,
        Integer,
        ForeignKey(f"{TableNames.USERS}.{ColumnNames.ID}", ondelete="CASCADE"),
        nullable=False,
    )
    title = Column(ColumnNames.TITLE, String, nullable=False)
    content_encrypted = Column(
        ColumnNames.CONTENT_ENCRYPTED, LargeBinary, nullable=False
    )
    personal_encryption = Column(
        ColumnNames.PERSONAL_ENCRYPTION, Boolean, default=False
    )
