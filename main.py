...
async def log_requests(request: Request, call_next):
    ...
    logger.info(f"%s %s %s user=%s %.1fms", request.method, request.url.path, response.status_code, user_id, duration_ms)
    return response
...
logger.info("Application setup is complete. Going to start the server if this is the main module.")