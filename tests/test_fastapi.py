...
def test_register_candidate():
    ...
    for candidate_id, candidate_name, party in candidates:
        candidate_data = {"id": candidate_id, "name": candidate_name, "party": party}
...
def test_vote_candidate():
    ...
    for user_id, candidate_id in votes:
        ...
        voting_data = {"candidate_id": candidate_id}
        headers = {"Authorization": f"Bearer {token}"}
        response = client.post(
            f"/users{Endpoints.VOTING}", json=voting_data, headers=headers
        )
        assert response.status_code == 201
        assert "id" in response.json()
...
def test_get_vote_counts():
    ...
    for vote_count in response.json():
        assert (
            ...
            == vote_counts[vote_count["candidate_id"]]["candidate_id"]
        )
        assert (
            vote_count["candidate_name"]
            == vote_counts[vote_count["candidate_id"]]["candidate_name"]
        )
        assert (
            vote_count["vote_count"]
            == vote_counts[vote_count["candidate_id"]]["vote_count"]
        )
