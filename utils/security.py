...
from db.DBModels import TokenBlacklist, UserDBModel
...
def hash_context() -> CryptContext:
    ...
    return CryptContext(
        schemes=["bcrypt", "pbkdf2_sha256", "argon2"], deprecated="auto"
    )
...
def create_access_token(data: dict) -> str:
    ...
    data_to_encode.update(
        {"exp": expire_time.timestamp(), "jti": jti}
    )  # Add expiration and unique token id
...
def decode_access_token(
    ...
    try:
        ...
        if not email:
            ...
            raise JWTError(
                status_code=401, detail=ResponseMessages.INVALID_TOKEN_MISSING_EMAIL
            )
        # Blacklist check
        jti = payload.get("jti")
        if jti:
        ...
        if not user:
            raise JWTError(
                status_code=401, detail=ResponseMessages.INVALID_TOKEN_MISSING_USER
            )
        return payload
